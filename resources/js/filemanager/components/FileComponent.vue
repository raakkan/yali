<template>
    <div @click="handleClick" :class="['flex flex-col items-center w-16 md:w-20 p-2 rounded-lg transition-colors cursor-pointer relative group',
        ...
        <div v-else-if="isVideo" class="w-12 h-12 md:w-16 md:h-16 overflow-hidden">
            <Video :file="file" />
        </div>
        ...
        <span class="mt-2 text-sm font-medium text-gray-700 text-center break-all line-clamp-2">{{ file.name }}</span>
...
import Image from './Files/Image.vue';
import Video from './Files/Video.vue';
...
export default defineComponent({
    ...
    setup(props) {
        ...
        return { isSelected, handleClick, selectFile, isFileSelected, removeFile, isSelectable, isImage, isVideo };
    }
});
</script>
